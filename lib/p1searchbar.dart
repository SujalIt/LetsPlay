...
import 'package:letsplay/Lets_Play_Api.dart';
...
class _p1searchbarState extends State<p1searchbar> {
   ...
   List<LetsPlayApi> main_list = [
    ];
Future<List<LetsPlayApi>>contents() async {
  final result=await http.get(Uri.parse('https://gmoflxgrysuxaygnjemp.supabase.co/rest/v1/vendor'));
  var output=jsonDecode(result.toString());
  if (result.statusCode==200){
    for(Map i in output){
      main_list.add(LetsPlayApi.fromJson(i));
    }
    print(main_list);
    return main_list;
  }else{
  ...
  // List<List<LetsPlayApi>> display_list = List.from(main_list);
